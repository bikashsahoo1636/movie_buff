module.exports = (function () {
    ...
    function getWishList(res, mysql, context, complete, id) {
      ...
      var sql =
        "SELECT Movie.movie_title FROM User_Movie_Detail JOIN Movie ON Movie.movie_id = User_Movie_Detail.movie_id AND User_Movie_Detail.user_id = ? and in_wishList=1";
      var inserts = [id];
    ...
    function getWatchList(res, mysql, context, complete, id) {
        // var sql = "SELECT movie_title, score, explanation, ratingID From Rating_Entry Inner Join Movie on Rating_Entry.movie_id=Movie.movie_id WHERE account_ID = ?";
        var sql =
          "SELECT Movie.movie_title FROM User_Movie_Detail JOIN Movie ON Movie.movie_id = User_Movie_Detail.movie_id AND User_Movie_Detail.user_id = ? and is_watched=1";
        var inserts = [id];
        mysql.pool.query(sql, inserts, function (error, results, fields) {
          if (error) {
            res.write(JSON.stringify(error));
